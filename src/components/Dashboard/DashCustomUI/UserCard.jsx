...
const UserCard = ({ user, editingUser, onEdit, onSave, onDelete, animating }) => {
  ...
  return (
    <div
      className={`bg-white rounded-xl shadow-lg border border-gray-200 p-6 transition-all duration-300 hover:shadow-xl hover:-translate-y-1 ${
      ...
      {editingUser?.id === user.id ? (
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            />
            {/* ✅ Use Select instead of plain <select> */}
            <Select
              value={localEdit.role}
              onValueChange={(val) => setLocalEdit({ ...localEdit, role: val })}
            >
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          ...
          <div className="flex gap-2 items-center">
            ...
            <Select
              value={user.role}
              onValueChange={(val) => onSave({ ...user, role: val })}
            >
            ...
            <button onClick={onDelete} className="px-3 py-2 text-red-600">
